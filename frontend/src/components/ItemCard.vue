<template>
  ...
  <div
    ...
    <div class="flex items-start gap-4 mb-4">
      <span class="text-5xl flex-shrink-0 leading-none">{{ item.category?.icon || getCategoryIcon(item.category?.name)
        }}</span>
      <div class="flex-1 min-w-0">
        <h3 class="text-xl font-black text-gray-900 leading-tight mb-2">{{ item.name }}</h3>
        <span class="inline-block text-2xl font-black text-primary-600">${{ item.price.toFixed(2) }}</span>
      </div>
    </div>

    <!-- Description -->
    <p v-if="item.description" class="text-gray-800 text-base mb-4 line-clamp-3 leading-relaxed font-medium">
      {{ item.description }}
    </p>
    ...
    <div v-if="item.tags && item.tags.length > 0" class="flex flex-wrap gap-2">
      <span v-for="tag in item.tags" :key="tag.id"
        class="inline-block px-3 py-1.5 rounded-full text-xs font-black border-2" :style="{
